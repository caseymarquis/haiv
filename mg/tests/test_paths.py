...
class TestPaths:
    ...
    def test_called_from_raises_if_none(self, tmp_path):
        ...
        paths = Paths(_called_from=None, _pkg_root=tmp_path, _hv_root=None)
        with pytest.raises(RuntimeError, match="called_from not set"):
            _ = paths.called_from

    def test_root(self, paths, tmp_path):
        """root returns hv_root."""
        assert paths.root == tmp_path

    def test_root_raises_if_none(self, tmp_path):
        """root raises if hv_root not set."""
        paths = Paths(_called_from=tmp_path, _pkg_root=tmp_path, _hv_root=None)
    ...
    def test_pkg_raises_if_none(self, tmp_path):
        ...
        paths = Paths(_called_from=tmp_path, _pkg_root=None, _hv_root=None)
    ...
    def test_project_root_derived_from_root(self, paths, tmp_path):
        ...
        assert paths.pkgs.project.root == tmp_path / "src" / "hv_project"

    def test_project_assets(self, paths, tmp_path):
        """project.assets_dir points to project's __assets__."""
        assert paths.pkgs.project.assets_dir == tmp_path / "src" / "hv_project" / "__assets__"


# TODO: Add paths.user after user identification is implemented
# paths.user will point to users/{current_user}/src/hv_user/


class TestIsValidHvRoot:
    """Tests for _is_valid_hv_root helper."""

    def test_valid_when_has_git_and_worktrees(self, tmp_path):
        """Valid haiv root has .git and worktrees directories."""
        from haiv.paths import _is_valid_hv_root

        (tmp_path / ".git").mkdir()
        (tmp_path / "worktrees").mkdir()
        assert _is_valid_hv_root(tmp_path) is True
    ...
    def test_invalid_when_missing_git(self, tmp_path):
        ...
        (tmp_path / "worktrees").mkdir()
        assert _is_valid_hv_root(tmp_path) is False
...
class TestGetHvRoot:
    """Tests for get_hv_root function."""

    @pytest.fixture
    def valid_hv_root(self, tmp_path):
    ...
    def test_returns_env_var_when_valid(self, valid_hv_root, monkeypatch):
        """Returns HV_ROOT env var when set and valid."""
        from haiv._infrastructure import env
        from haiv.paths import get_hv_root

        monkeypatch.setenv(env.HV_ROOT, str(valid_hv_root))
        assert get_hv_root(cwd=Path("/some/other/path")) == valid_hv_root
    ...
    def test_raises_if_env_not_valid_hv_root(self, tmp_path, monkeypatch):
    ...
    def test_finds_hv_root_in_cwd(self, valid_hv_root, monkeypatch):
        """Finds haiv root when cwd is the root."""
        from haiv._infrastructure import env
        from haiv.paths import get_hv_root

        monkeypatch.delenv(env.HV_ROOT, raising=False)
        assert get_hv_root(cwd=valid_hv_root) == valid_hv_root

    def test_finds_hv_root_in_parent(self, valid_hv_root, monkeypatch):
        """Finds haiv root by walking up from subdirectory."""
        from haiv._infrastructure import env
        from haiv.paths import get_hv_root

        subdir = valid_hv_root / "some" / "nested" / "dir"
        subdir.mkdir(parents=True)

        monkeypatch.delenv(env.HV_ROOT, raising=False)
        assert get_hv_root(cwd=subdir) == valid_hv_root

    def test_raises_when_no_hv_root_found(self, tmp_path, monkeypatch):
    ...
    def test_error_suggests_hv_start(self, tmp_path, monkeypatch):
