"""Background worker that gathers recent commits across worktrees.

Runs on its own thread. Watches the worktrees directory for changes and
re-gathers on edits — commits typically follow file changes.
"""
...
from haiv.helpers.tui.recent_commits import gather_recent_commits
from haiv.helpers.utils.file_watcher import FileWatcher
from haiv_tui.command_filters import is_claude_hook_stop


class RecentCommitsWorker:
    """Background thread that keeps recent commits up to date.
    ...
    def start(self) -> RecentCommitsWorker:
        self._thread = threading.Thread(target=self._run, daemon=True, name="recent-commits")
        self._thread.start()
        ...
        if self._worktrees_dir.is_dir():
            ...
            self._watcher = (
                FileWatcher(_on_change, debounce_seconds=5.0)
    ...
    def _gather_and_push(self) -> None:
        try:
            commits = gather_recent_commits(self._worktrees_dir)
            self._client.write_raw(recent_commits=commits)
        except Exception:
            pass
