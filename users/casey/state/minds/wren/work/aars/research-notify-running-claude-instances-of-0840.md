...
- Implementation: `hv pop` AAR scaffolding (template creation) and parent notification on `--session`
...
- **Template scaffolded on `hv pop`** — created automatically with `skip_existing`, so re-running pop doesn't overwrite a partially filled AAR.
...
- **Process gap: testing hv commands from worktrees** — workers can't run modified `hv` commands during development, only the installed version
...
Cleared `CLAUDECODE` env var in `generate_mind_name()` subprocess call so `hv minds stage` works from inside a Claude Code session. (`haiv/src/haiv/helpers/minds.py:386`)

## Commits and Files Changed

- d3b778b feat(hv): add Session.as_filename() for filesystem-safe task names
