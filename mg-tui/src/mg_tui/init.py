...
def resolve_paths(on_error: Callable[[str], None]) -> Paths | None:
    ...
    try:
        hv_root = get_hv_root(Path.cwd())
    except ValueError as e:
        on_error(f"paths: {e}")
        return None

    user = detect_user(hv_root / "users")
    if user is None:
        on_error(
            f"No user identity found (hv_root={hv_root}). "
            "Run 'hv users new --name <name>'."
        )
        return None

    return Paths(_called_from=None, _pkg_root=None, _hv_root=hv_root, _user_name=user.name)


def load_settings(paths: Paths, on_error: Callable[[str], None]) -> HvSettings:
...
def init(on_error: Callable[[str], None]) -> HvDeps:
    ...
    settings = load_settings(paths, on_error) if paths else HvSettings()
    ...
    return HvDeps(paths=paths, settings=settings, terminal=terminal)
