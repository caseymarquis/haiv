...
class Mind:
    ...
    def get_startup_files(self) -> list[Path]:
        ...
        Raises:
            RuntimeError: If haiv_root not set and references.toml has entries.
        """
        files: list[Path] = []

        # Add resolved reference paths first
        for ref_path in self.get_references():
            if self.paths.haiv_root is None:
                raise RuntimeError(
                    f"Cannot resolve reference path '{ref_path}': haiv_root not set on MindPaths"
                )
            files.append(self.paths.haiv_root / ref_path)
...
def resolve_mind(name: str, minds_dir: Path, haiv_root: Path) -> Mind:
    ...
    for mind_name, mind_path in all_minds:
        if mind_name == name:
            return Mind(paths=MindPaths(root=mind_path, haiv_root=haiv_root))

    raise MindNotFoundError(name, available)


def list_minds(minds_dir: Path, haiv_root: Path) -> list[Mind]:
    ...
    Raises:
        DuplicateMindError: If duplicate names exist.
    """
    return [Mind(paths=MindPaths(root=path, haiv_root=haiv_root)) for _, path in list_mind_paths(minds_dir)]
