...
class SettingsCache:
    ...
    project: HaivSettings | None = None
    user: HaivSettings | None = None


def get_settings(paths: Paths, cache: SettingsCache) -> HaivSettings:
...
def _load_settings(path: Path, default_text: str) -> HaivSettings:
    ...
    Returns:
        HaivSettings with loaded values.
    """
    if not path.exists():
    ...
    # Build HaivSettings from TOML data by matching field names.
    # Field "_foo" is loaded from TOML key "foo".
    from dataclasses import fields

    kwargs = {}
    for field in fields(HaivSettings):
    ...
    return HaivSettings(**kwargs)


def load_project_settings(path: Path) -> HaivSettings:
...
def load_user_settings(path: Path) -> HaivSettings:
    ...
    If file is missing, creates an empty file and returns empty HaivSettings.
    """
...
def merge_settings(project: HaivSettings, user: HaivSettings | None) -> HaivSettings:
