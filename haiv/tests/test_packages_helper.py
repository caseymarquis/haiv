...
class TestDiscoverPackages:
    ...
    def test_project_local_included_when_valid(self, tmp_path):
        """Project package included when src/haiv_project/commands/__init__.py exists."""
        ...
        assert project.name == "haiv_project"
        assert project.paths.root == tmp_path / "src" / "haiv_project"

    def test_project_local_omitted_when_no_init(self, tmp_path):
        """Project package omitted when commands/__init__.py missing."""
        (tmp_path / "src" / "haiv_project" / "commands").mkdir(parents=True)
        # No __init__.py
    ...
    def test_project_local_omitted_when_no_commands(self, tmp_path):
        ...
        (tmp_path / "src" / "haiv_project").mkdir(parents=True)
    ...
    def test_project_local_omitted_when_no_haiv_project(self, tmp_path):
        """Project package omitted when src/haiv_project/ doesn't exist."""
        result = discover_packages(tmp_path)
    ...
    def test_user_local_included_when_valid(self, tmp_path):
        ...
        assert user_pkg.name == "haiv_user"
        assert user_pkg.paths.root == user.paths.haiv_user.root
...
class TestDiscoverPackagesDetailed:
    ...
    def test_skipped_has_reasons(self, tmp_path):
        ...
        # Don't create haiv_project - it will be skipped
    ...
    def test_haiv_root_none_skips_project_and_user(self):
        """When haiv_root is None, project and user are skipped."""
        result = discover_packages_detailed(haiv_root=None)
        ...
        assert project[0].reason == "haiv_root not provided"
        assert len(user) == 1
        assert user[0].reason == "haiv_root not provided"
