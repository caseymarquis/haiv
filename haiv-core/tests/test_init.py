...
class TestInitFreshEmpty:
    ...
    def test_creates_haiv_project_package(self, fresh_init):
        haiv_project = fresh_init.paths.root / "src" / "haiv_project"
        assert haiv_project.is_dir()
        assert (haiv_project / "__init__.py").exists()
    ...
    def test_creates_haiv_hq_orphan_branch(self, fresh_init):
        assert fresh_init.git.branch_current() == "haiv-hq"
...
class TestInitPeerBasic:
    ...
    def test_peer_named_with_haiv_suffix(self, in_repo):
        in_repo.init()
    ...
    def test_peer_has_haiv_hq_orphan_branch(self, in_repo):
        in_repo.init()

        git = Git(in_repo.peer_dir)
        assert git.branch_current() == "haiv-hq"
