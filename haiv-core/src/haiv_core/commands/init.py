"""Initialize haiv-hq branch in current directory."""
...
def define() -> cmd.Def:
    ...
    return cmd.Def(
        description="Initialize haiv-hq branch in current directory",
        flags=[
...
def _write_haiv_state_files(root: Path, ctx: cmd.Ctx) -> None:
    ...
    commands = haiv_project / "commands"
    ctx.templates.write("init/src/haiv_project/commands/__init__.py.j2", commands / "__init__.py")
...
def _print_next_steps(ctx: cmd.Ctx, *, branch: str, quiet: bool) -> None:
    ...
    ctx.print(f"Use 'hv worktree add <branch>' to add worktrees.")
...
def _init_fresh_mode(
    ...
    *,
    ...
    """Initialize haiv-hq in a directory that's not in a git repo."""
...
def _init_fresh_empty(
    ...
    *,
    ...
    """Initialize haiv-hq in an empty directory."""
    root = ctx.paths.called_from
    git = Git(root, quiet=quiet)

    _init_haiv_structure(git, ctx)
...
def _init_fresh_nonempty(
    ...
    *,
    ...
    """Initialize haiv-hq in a non-empty directory (--force required)."""
    root = ctx.paths.called_from
    git = Git(root, quiet=quiet)
...
def _init_peer_mode(
    ...
    *,
    ...
    """Initialize haiv-hq as a peer to an existing git repo."""
